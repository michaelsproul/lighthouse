...
pub struct Context<T: SlotClock, E, S> {
    pub task_executor: TaskExecutor,
    pub api_secret: ApiSecret,
    pub block_service: Option<BlockService<LighthouseValidatorStore<T, E, S>, T>>,
    pub validator_store: Option<Arc<LighthouseValidatorStore<T, E, S>>>,
...
/// configuration.
pub fn serve<T: 'static + SlotClock + Clone, E: EthSpec, S: SlashingDatabase + Send + Sync + 'static>(
    ctx: Arc<Context<T, E, S>>,
    ...
    let get_lighthouse_validators = warp::path("lighthouse")
        ...
        .then(|validator_store: Arc<LighthouseValidatorStore<T, E, S>>| {
            blocking_json_task(move || {
    ...
    let get_lighthouse_validators_pubkey = warp::path("lighthouse")
        ...
        .and(validator_store_filter.clone())
        .then(
            |validator_pubkey: PublicKey, validator_store: Arc<LighthouseValidatorStore<T, E, S>>| {
                blocking_json_task(move || {
    ...
    let get_lighthouse_beacon_health = warp::path("lighthouse")
        ...
        .then(
            |block_filter: BlockService<LighthouseValidatorStore<T, E, S>, T>| async move {
    ...
    let post_lighthouse_beacon_update = warp::path("lighthouse")
        ...
        .then(
            move |request: UpdateCandidatesRequest,
                  block_service: BlockService<LighthouseValidatorStore<T, E, S>, T>| async move {
