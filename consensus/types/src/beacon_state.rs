...
impl<T: EthSpec> BeaconState<T> {
    ...
    pub fn latest_execution_payload_header(&self) -> Result<ExecutionPayloadHeader<T>, Error> {
        match self {
            Self::Base(_) | Self::Altair(_) => Err(Error::IncorrectStateVariant),
            Self::Merge(state) => Ok(state.latest_execution_payload_header.clone().into()),
            Self::Capella(state) => Ok(state.latest_execution_payload_header.clone().into()),
        }
    }
