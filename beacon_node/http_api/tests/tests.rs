...
impl ApiTester {
    ...
    pub async fn test_get_debug_beacon_states(self) -> Self {
        ...
        for state_id in self.interesting_state_ids() {
            ...
            assert_eq!(result_json.data, expected, "{:?}", state_id);
            assert_eq!(
                result_json.version,
                Some(expected.fork_name(&self.chain.spec).unwrap())
            );
            ...
            let result_v1 = self
                .client
                .get_debug_beacon_states_v1(state_id, &self.chain.spec)
                .await
                .unwrap();
            assert_eq!(result_v1.version, None);
            assert_eq!(result_v1.data, expected, "{:?}", state_id);
        }
        */
